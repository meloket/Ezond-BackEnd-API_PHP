...
function merge_calls($arr1, $arr2)
{
    return array_merge($arr1->calls, $arr2->calls);
}


$result_json_data = json_decode("{}");
function send_curlu($page = 1)
{
    global $api_key, $account_id, $start_date, $end_date, $result_json_data;

    $page_count = 1000;
    // $result_json_data = json_decode("{}");

    while ($page <= $page_count) {
        //$api_url = "https://api.callrail.com/v1/calls.json?per_page=250&page=" . $page . "&company_id=" . $account_id . "&start_date=" . $start_date . "&end_date=" . $end_date;

        $api_url = "https://api.callrail.com/v2/companies/".$account_id."/calls.json?per_page=250&page=".$page."&start_date=".$start_date."&end_date=".$end_date;
        ...
        if (!$result_json_data->calls) {
            $result_json_data = json_decode($json_data);
        } else {
            $result_json_data->calls = merge_calls($result_json_data, json_decode($json_data));
        }
        curl_close($ch);
...
for ($i = 0; $i < count($arrCalls); $i++) {
    ...
    $callObj["Name"] = $arrCalls[$i]->customer_name;
    ...
    $callObj["Phone Number"] = $arrCalls[$i]->customer_phone_number;
    $callObj["Duration"] = $arrCalls[$i]->duration . "s";
    $callObj["Location"] = $arrCalls[$i]->customer_country;
