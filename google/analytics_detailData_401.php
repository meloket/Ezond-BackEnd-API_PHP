...
$response = getReport($analytics, $viewID, array($medium), false, "ga:landingPagePath");
$result = printResults($response);
for ($i = 0; $i < count($result); $i++) {
    if (strlen($result[$i]->dimensions) > 60)
        $result[$i]->Page = substr($result[$i]->dimensions, 0, 60);
    else
        $result[$i]->Page = $result[$i]->dimensions;
}
$ret->result = $result;
