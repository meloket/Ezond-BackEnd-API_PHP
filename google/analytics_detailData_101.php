...
$response = getReport($analytics, $viewID, array("Organic Search"));
$result = printTotals($response);
if (count($result) > 0) $ret = $result[0];

$response = getReport($analytics, $viewID, array("Organic Search"), false, "ga:keyword");
$result = printResults($response);
for ($i = 0; $i < count($result); $i++)
    $result[$i]->Keyword = $result[$i]->dimensions;
$ret->result = $result;
...
$response = getReport($analytics, $viewID, array("Organic Search"), true);
$result = printResults($response);
for ($i = 0; $i < count($result); $i++) {
    foreach ($arr_alias as $key => $alias) {
