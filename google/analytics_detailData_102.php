...
$response = getReport($analytics, $viewID, array("Paid Search"));
$result = printTotals($response);
if (count($result) > 0) $ret = $result[0];

$response = getReport($analytics, $viewID, array("Paid Search"), false, "ga:keyword");
...
$response = getReport($analytics, $viewID, array("Paid Search"), true);
$result = printResults($response);
for ($i = 0; $i < count($result); $i++) {
    foreach ($arr_alias as $key => $alias) {
