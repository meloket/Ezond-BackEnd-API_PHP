...
class Mysql extends \PDO
{
    ...
    {
        $connectPrefix = DB_TYPE . ':host=' . DB_HOST . ';dbname=' . DB_NAME;
        if(defined('DB_PORT')) {
            $connectPrefix .= (';port=' . DB_PORT);
        }
    ...
    public function prepareDataForSqlInClause(array $data, string $keyPrefix)
    {
        $inData = $namedKeys = [];

        foreach ($data as $key => $value) {
            $namedKey = $keyPrefix . $key;
            $inData[$namedKey] = $value;
            $namedKeys[] = ':' . $namedKey;
        }
        $inStr = implode(',', $namedKeys);

        return ['inStr' => $inStr, 'inData' => $inData];
    }
