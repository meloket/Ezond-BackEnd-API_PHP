...
<?= print_r($data, true) ?>
      ...
      <ol>
        ...
        <li>Create a standard formatted API Key using the OAuth2 access token (<?= $session['access_token'] ?>) and the
          data center (<?= $data['dc'] ?>) :
          <strong><?= $session['access_token'] ?>-<?= $data['dc'] ?></strong> which can be passed into most API
          wrappers.
        </li>
      </ol>
      <h2>Want to see an authorization fail?</h2>
      The <strong>code</strong> being exchange for an OAuth Access Token expires within 30 seconds - simply refresh this
      page in a minute.
