...
$arrHeaderFlds = ['Ad', 'Headline', 'HeadlinePart1', 'HeadlinePart2', 'CreativeDestinationUrl', 'Ad Group', 'Status', 'Campaign', 'Network'];

$arrOriginFld = ['Clicks', 'Impressions', 'Cost', 'Average CPC', 'CTR', 'Conversions', 'Cost Per Conversion', 'Conversion Rate', 'View-Through Conv.', 'Avg. Position'];

$arrFlds = ['Description', 'Headline', 'HeadlinePart1', 'HeadlinePart2', 'CreativeDestinationUrl', 'AdGroupName', 'AdGroupStatus', 'CampaignName', 'AdNetworkType1', 'Clicks', 'Impressions', 'Cost', 'AverageCpc', 'Ctr', 'Conversions', 'CostPerConversion', 'ConversionRate', 'ViewThroughConversions', 'AveragePosition'];

$arrFlds_date = ['Date', 'Clicks', 'Impressions', 'Cost', 'AverageCpc', 'Ctr', 'Conversions', 'CostPerConversion', 'ConversionRate', 'ViewThroughConversions', 'AveragePosition'];

$filter_obj = new stdClass();
$filter = $_GET['filter'];
$arrFilter = explode("@", $filter);

if ($arrFilter[0] != "All Campaigns")
    $filter_obj->campaignName = $arrFilter[0];

if ($arrFilter[1] != "All Adgroups")
    $filter_obj->groupName = $arrFilter[1];

if ($arrFilter[2] == "Search Network")
    $filter_obj->networkName = "SEARCH";
else if ($arrFilter[2] == "Display Network")
    $filter_obj->networkName = "DISPLAY";

$result = GetReport($session, $start_date, $end_date, ReportDefinitionReportType::AD_PERFORMANCE_REPORT, $arrFlds, $filter_obj);
...
for ($i = 0; $i < count($result); $i++) {
    if (trim($result[$i]->Ad) == "--") $result[$i]->Ad = "";
    $str = "";
    if (trim($result[$i]->Headline) != "--") $str .= $result[$i]->Headline . " ";
    if (trim($result[$i]->HeadlinePart1) != "--") $str .= $result[$i]->HeadlinePart1 . " ";
    if (trim($result[$i]->HeadlinePart2) != "--") $str .= $result[$i]->HeadlinePart2 . " ";
    if ($str != "") $result[$i]->Ad = $str . $result[$i]->Ad;
    if (strlen($result[$i]->Ad) > 70) $result[$i]->Ad = substr($result[$i]->Ad, 0, 70) . "...";
}
$ret->result = $result;
